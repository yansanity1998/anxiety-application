...
const Schedule = ({ darkMode }: ScheduleProps) => {
  ...
  const showStudentInfo = (appointment: Appointment) => {
    ...
    modal.className = 'fixed inset-0 bg-black/20 backdrop-blur-sm flex items-start justify-center z-50 p-4 overflow-y-auto';
    modal.innerHTML = `
  ...
  const handleDeleteAppointment = async (appointment: Appointment) => {
    const confirmed = await new Promise<boolean>((resolve) => {
      const modal = document.createElement('div');
      modal.className = 'fixed inset-0 bg-white/20 backdrop-blur-md flex items-start justify-center z-50 p-4 overflow-y-auto';
      modal.innerHTML = `
