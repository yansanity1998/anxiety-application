  ...
  return (
    <div className={`${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-xl shadow-lg p-4 sm:p-6 border ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        <div className={`p-3 rounded-xl ${darkMode ? 'bg-gray-700/50' : 'bg-gray-50/50'} border ${darkMode ? 'border-gray-600' : 'border-gray-200'} backdrop-blur-sm`}>
          <div className="text-center">
            <div className={`text-lg sm:text-xl font-bold ${darkMode ? 'text-white' : 'text-gray-900'}`}>{stats.total}</div>
            <div className={`text-xs ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>Total</div>
          </div>
        </div>
        <div className="p-3 rounded-xl bg-green-50/50 border border-green-200 backdrop-blur-sm">
          <div className="text-center">
            <div className="text-lg sm:text-xl font-bold text-green-600">{stats.completed}</div>
            <div className="text-xs text-green-600">Completed</div>
          </div>
        </div>
        <div className="p-3 rounded-xl bg-blue-50/50 border border-blue-200 backdrop-blur-sm">
          <div className="text-center">
            <div className="text-lg sm:text-xl font-bold text-blue-600">{stats.inProgress}</div>
            <div className="text-xs text-blue-600">In Progress</div>
          </div>
        </div>
        <div className="p-3 rounded-xl bg-red-50/50 border border-red-200 backdrop-blur-sm">
          <div className="text-center">
            <div className="text-lg sm:text-xl font-bold text-red-600">{todos.filter(t => t.status === 'canceled').length}</div>
