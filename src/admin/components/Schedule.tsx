...
const Schedule = ({ darkMode }: ScheduleProps) => {
  ...
  const showStudentInfo = (appointment: Appointment) => {
    ...
    modal.innerHTML = `
      <div class="${darkMode ? 'bg-gradient-to-br from-gray-900 to-gray-800 border-[#800000]/50' : 'bg-white border-gray-200'} rounded-xl shadow-xl w-full max-w-md max-h-[95vh] sm:max-h-[90vh] overflow-y-auto border">
        <div class="p-3 sm:p-4 lg:p-5">
          <div class="flex items-center gap-3 sm:gap-4 mb-4 sm:mb-5">
            ...
            <div class="flex-1 min-w-0">
              <h3 class="text-base sm:text-lg font-semibold ${darkMode ? 'text-white' : 'text-gray-900'} truncate">${appointment.student_name}</h3>
              <p class="text-xs sm:text-sm ${darkMode ? 'text-gray-400' : 'text-gray-500'} truncate">${appointment.student_email}</p>
            </div>
            <button 
          ...
          <div class="space-y-3 sm:space-y-4">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
              <div class="p-2 sm:p-3 rounded-lg ${darkMode ? 'bg-gray-800/50 border-gray-700' : 'bg-gray-50 border-gray-100'} border">
                <div class="flex items-center gap-2 mb-1 sm:mb-1.5">
                  ...
                  <span class="text-[10px] sm:text-xs font-medium ${darkMode ? 'text-gray-400' : 'text-gray-600'}">Date</span>
                </div>
                <p class="text-xs sm:text-sm font-semibold ${darkMode ? 'text-white' : 'text-gray-900'}">${formatDate(appointment.appointment_date)}</p>
              </div>
              ...
              <div class="p-2 sm:p-3 rounded-lg ${darkMode ? 'bg-gray-800/50 border-gray-700' : 'bg-gray-50 border-gray-100'} border">
                <div class="flex items-center gap-2 mb-1 sm:mb-1.5">
                  ...
                  <span class="text-[10px] sm:text-xs font-medium ${darkMode ? 'text-gray-400' : 'text-gray-600'}">Time</span>
                </div>
                <p class="text-xs sm:text-sm font-semibold ${darkMode ? 'text-white' : 'text-gray-900'}">${appointment.appointment_time}</p>
              </div>
            </div>
            ...
            <div class="p-2 sm:p-3 rounded-lg ${darkMode ? 'bg-gray-800/50 border-gray-700' : 'bg-gray-50 border-gray-100'} border">
              <div class="flex items-center gap-2 mb-1 sm:mb-1.5">
                ...
                <span class="text-[10px] sm:text-xs font-medium ${darkMode ? 'text-gray-400' : 'text-gray-600'}">Status</span>
              </div>
            ...
            ${appointment.notes ? `
              <div class="p-2 sm:p-3 rounded-lg ${darkMode ? 'bg-gray-800/50 border-gray-700' : 'bg-gray-50 border-gray-100'} border">
                <div class="flex items-center gap-2 mb-1 sm:mb-1.5">
                  ...
                  <span class="text-[10px] sm:text-xs font-medium ${darkMode ? 'text-gray-400' : 'text-gray-600'}">Notes</span>
                </div>
                <p class="text-xs sm:text-sm ${darkMode ? 'text-gray-300' : 'text-gray-700'} leading-relaxed">${appointment.notes}</p>
              </div>
            ` : ''}
          </div>
          ...
          <div class="flex flex-col sm:flex-row gap-2 mt-4 sm:mt-5">
            <button 
              ...
              class="flex-1 flex items-center justify-center gap-1.5 px-3 py-1.5 ${darkMode ? 'bg-gray-700 hover:bg-gray-600 text-gray-200' : 'bg-gray-100 hover:bg-gray-200 text-gray-700'} text-sm font-medium rounded-lg transition-colors"
            >
  ...
  const handleDeleteAppointment = async (appointment: Appointment) => {
    const confirmed = await new Promise<boolean>((resolve) => {
      ...
      const modal = document.createElement('div');
      modal.className = 'fixed inset-0 bg-black/20 backdrop-blur-md flex items-center justify-center z-50 p-2 sm:p-4';
      modal.innerHTML = `
        <div class="${darkMode ? 'bg-gradient-to-br from-gray-900 to-gray-800 border-red-600/50' : 'bg-white border-red-200'} rounded-2xl shadow-2xl w-full max-w-sm mx-2 sm:mx-4 border">
          <div class="p-4 sm:p-6">
            <div class="text-center space-y-3 sm:space-y-4">
              <div class="inline-flex items-center justify-center w-12 h-12 sm:w-16 sm:h-16 ${darkMode ? 'bg-red-900/30' : 'bg-red-100'} rounded-full mb-2 sm:mb-3">
                <svg class="w-6 h-6 sm:w-8 sm:h-8 ${darkMode ? 'text-red-400' : 'text-red-600'}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                </svg>
              </div>
              <h3 class="text-base sm:text-lg font-bold ${darkMode ? 'text-white' : 'text-gray-900'}">Delete Appointment?</h3>
              <p class="text-sm sm:text-base ${darkMode ? 'text-gray-300' : 'text-gray-600'}">Are you sure you want to delete this appointment?</p>
              <div class="${darkMode ? 'bg-red-900/30 border-red-600/40' : 'bg-red-50 border-red-200'} p-3 sm:p-4 rounded-xl border">
                <p class="font-semibold ${darkMode ? 'text-red-300' : 'text-red-800'} text-base sm:text-lg">${appointment.student_name}</p>
                <p class="text-xs sm:text-sm ${darkMode ? 'text-red-400' : 'text-red-600'}">${formatDate(appointment.appointment_date)} at ${appointment.appointment_time}</p>
              </div>
              <p class="text-xs sm:text-sm ${darkMode ? 'text-gray-400' : 'text-gray-500'}">This action cannot be undone.</p>
              <div class="flex flex-col sm:flex-row gap-2 sm:space-x-3 mt-4 sm:mt-6">
                <button id="cancelBtn" class="flex-1 ${darkMode ? 'bg-gray-700 text-gray-200 hover:bg-gray-600' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'} px-4 py-2 rounded-xl font-medium active:scale-95 transition-all text-sm sm:text-base">Cancel</button>
